...
use App\Models\Legacy\LegacyClientDebtPayment;
...
class ClientDebts extends Component
{
    use WithPagination;
    ...
    public function openDebtModal(?int $id = null): void
    {
        ...
        if ($id) {
            $debt = LegacyClientDebt::findOrFail($id);
            $this->client_id = (string) $debt->client_id;
    ...
    public function closeDebtModal(): void
    {
        ...
        $this->reset(['debtId', 'client_id', 'devise_id', 'due_amount', 'debt_date', 'notes']);
        $this->resetValidation();
    }
    ...
    public function saveDebt(): void
    {
        ...
        if ($this->debtId) {
            $debt = LegacyClientDebt::findOrFail($this->debtId);
            if ($debt->is_closed) {
    ...
    public function openPaymentModal(int $debtId): void
    {
        ...
        $debt = LegacyClientDebt::findOrFail($debtId);
        if ($debt->is_closed) {
    ...
    {
        return 'LCP-' . now()->format('ymd') . '-' . random_int(100000, 999999);
    }
    ...
    public function savePayment(): void
    {
        ...
        $debt = LegacyClientDebt::findOrFail((int) $this->paymentDebtId);
        if ($debt->is_closed) {
            session()->flash('error', 'Cette dette est déjà clôturée.');
            return;
        }
        ...
        DB::transaction(function () use ($debt, $amount) {
            ...
            while (LegacyClientDebtPayment::where('reference', $reference)->exists()) {
                $reference = $this->generatePaymentReference();
            }

            LegacyClientDebtPayment::create([
                'legacy_client_debt_id' => $debt->id,
    ...
    {
        view()->share('title', 'Anciens - Dettes clients');
        view()->share('breadcrumb', 'Anciens / Dettes clients');

        $baseQuery = LegacyClientDebt::query()
            ->with(['client', 'devise'])
        ...
        if ($this->showPaymentModal && $this->paymentDebtId) {
            $paymentDebt = LegacyClientDebt::with(['client', 'devise'])->find($this->paymentDebtId);
            if ($paymentDebt) {
        ...
        if ($this->showViewModal && $this->viewDebtId) {
            $viewDebt = LegacyClientDebt::query()
                ->with(['client', 'devise'])
                ->withSum('payments as paid_sum', 'montant')
        ...
        return view('livewire.legacy.client-debts', [
            'debts' => $debts,
            'clients' => $clients,
            'devises' => $devises,
