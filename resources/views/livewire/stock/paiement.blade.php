    ...
    <div class="card border-0 shadow-sm mb-4">
        <div class="card-body">
            <div class="row g-3">
                ...
                <div class="col-md-2">
                    <select class="form-select" wire:model.live="filterDevise">
                        <option value="">Devise</option>
                        @foreach ($devises as $devise)
                            <option value="{{ $devise->id }}">{{ $devise->code }}
                                ({{ $devise->symbole ?? $devise->code }})</option>
                        @endforeach
                    </select>
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($reception)
                                        ...
                                        <span
                                            class="badge
                                        {{ $status === 'PAYE' ? 'bg-success' : ($status === 'PARTIEL' ? 'bg-warning' : 'bg-secondary') }}
                                ...
                                <td class="text-center pe-4">
                                    <div class="d-flex justify-content-center gap-2">
                                        <button class="btn btn-sm btn-outline-info"
                                            wire:click="showDetails({{ $paiement->id }})" data-bs-toggle="tooltip"
                                            title="Voir détails">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        <button class="btn btn-sm btn-outline-primary"
                                            wire:click="edit({{ $paiement->id }})" data-bs-toggle="tooltip"
                                            title="Modifier">
        ...
        {{-- ================= MODAL ================= --}}
        @if ($showModal)
            <div wire:show="showModal" x-transition.opacity.duration.200ms x-transition.scale.duration.200ms
                        ...
                        <form wire:submit.prevent="store">
                            ...
                            <div class="modal-body bg-white p-4">
                                ...
                                @if ($selectedReception)
                                    @php
                                        $currency =
                                            $selectedReception->commande?->devise?->symbole ??
                                            ($selectedReception->commande?->devise?->code ?? 'FC');
                                    @endphp
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select class="form-select @error('reception_id') is-invalid @enderror"
                                            wire:model="reception_id" {{ !$commande_id ? 'disabled' : '' }}>
                                            <option value="">Sélectionner Réception</option>
                                            @foreach ($receptions as $reception)
                                                <option value="{{ $reception->id }}">
                                                    {{ $reception?->reference }} -
                                                    Reste :
                                                    {{ number_format($reception->getRemainingAmount(), 0, ',', ' ') }}
                                                    {{ $reception->commande->devise->symbole ?? $reception->commande->devise->symbole }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('reception_id')
                                            ...
                                            @foreach ($modesPaiement as $value => $label)
                                                ...
                                                <div class="col-6 col-md-4">
                                                    <label
                                                        ...
                                                        <i
                                                            class="fas {{ $icons[$value] ?? 'fa-credit-card' }} text-primary"></i>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea class="form-control" rows="2" wire:model.defer="notes" placeholder="Commentaire du paiement..."></textarea>
                                    </div>
                                </div>
                            </div>
