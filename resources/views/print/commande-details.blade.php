@php
    ...
    $publicUrl = \Illuminate\Support\Facades\URL::signedRoute('public.commandes.show', [
...
<div class="p-4" id="printableArea">
    <div class="{{ $showHeader ? '' : 'd-none d-print-block' }} mb-4">
        ...
        <div class="text-center mt-3">
            ...
            <div class="mb-0">REF: {{ $selectedCommande->reference }}</div>
            <div class="mb-0">{{ \Carbon\Carbon::parse($selectedCommande->date_commande)->format('d/m/Y') }}</div>
        </div>
        <hr class="mt-3 mb-0">
    </div>
    ...
    <div class="row mb-4">
        ...
        <div class="col-md-6 text-end">
            ...
            <div>
                <span
                    class="badge bg-{{ $statusConfig['class'] }} bg-opacity-10 text-{{ $statusConfig['class'] }} border border-{{ $statusConfig['class'] }} border-opacity-25">
    ...
    <div class="row mb-4">
            ...
            <div class="card border h-100">
                <div class="card-body p-3">
                    <div class="text-center">
                        <div class="fw-bold h4 text-primary">{{ number_format($totalAfterDiscount, 0, ',', ' ') }}
                            {{ $currency }}</div>
        ...
        <div class="col-md-3">
            <div class="card border h-100">
                <div class="card-body p-3">
                    <div class="text-center">
                        <div class="fw-bold h4 text-success">{{ number_format($totalPaid, 0, ',', ' ') }}
                            {{ $currency }}</div>
        ...
        <div class="col-md-3">
            <div class="card border h-100">
                <div class="card-body p-3">
                    <div class="text-center">
                        <div class="fw-bold h4 {{ $remainingAmount > 0 ? 'text-warning' : 'text-success' }}">
                            {{ number_format($remainingAmount, 0, ',', ' ') }} {{ $currency }}</div>
        ...
        <div class="col-md-3">
            <div class="card border h-100">
                <div class="card-body p-3">
                    <div class="text-center">
                        <span
                            class="badge bg-{{ $paymentStatusConfig['class'] }} bg-opacity-10 text-{{ $paymentStatusConfig['class'] }} border border-{{ $paymentStatusConfig['class'] }} border-opacity-25 p-2">
                            <i class="fas {{ $paymentStatusConfig['icon'] }} me-1"></i>
                            {{ $paymentStatusConfig['label'] }}
                        </span>
                        <div class="progress mt-2" style="height: 8px;">
                            <div class="progress-bar bg-success" role="progressbar"
                                style="width: {{ $paymentProgress }}%" aria-valuenow="{{ $paymentProgress }}"
    ...
    <div class="{{ $showHeader ? '' : 'd-none d-print-block' }} mt-4 pt-3 border-top text-center small text-muted">
        <div>{{ $companyName }}</div>
        <div class="mt-1">REF: {{ $selectedCommande->reference }}</div>
    </div>
</div>
